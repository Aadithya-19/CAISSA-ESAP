...
ok so we can start writing here. Pretty straightforward. StP basically has an entry point and it looks like this.
    ...
    0 0 0 0 <- 1 (I load a bit in series (as i was rereading it, i said its something i load in serially lmfao))
    0 <- 0 0 0 1 <- 0 (The end bit gets shifted out, my end result has to be whatever I loaded and if i load something new in, it will change)
    0 <- 0 0 1 0 <- 0
...
same example as before. Parallel out becomes 0 0 1 0, nothing changes
